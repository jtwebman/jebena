...
"$JAVAC" -d "$OUT" "$ROOT/test/stress/ClqStress.java" "$ROOT/test/diff/Driver.java"
"$ZIG" build --build-file "$ROOT/build.zig" >/dev/null 2>&1
JEBENA="$ROOT/zig-out/bin/jebena"
mapfile -t JBASE < <(find "$ROOT/jbase/out" -name '*.class')
mapfile -t APP < <(ls "$OUT"/st/*.class)

EXP=$("$JAVA" -cp "$OUT" Driver st.ClqStress demo 2>/dev/null)
fail=0
# per-rep timeout retried ONCE (load-flake tolerance; a genuine hang fails both tries).
onerep() { # env rep
  local out rc got
  out=$(timeout 120 env $1 "$JEBENA" run st/ClqStress demo "${APP[@]}" "${JBASE[@]}" 2>&1); rc=$?
  if [ $rc -eq 124 ]; then
    out=$(timeout 120 env $1 "$JEBENA" run st/ClqStress demo "${APP[@]}" "${JBASE[@]}" 2>&1); rc=$?
    [ $rc -eq 124 ] && { echo "clq-stress: FAIL ($1) rep=$2 HANG (twice)"; fail=1; return; }
  fi
  got=$(printf '%s\n' "$out" | sed -n 's/.*demo() = \(-\?[0-9]*\).*/\1/p')
  [ "$got" = "$EXP" ] || { echo "clq-stress: FAIL ($1) rep=$2 jebena=$got java=$EXP"; fail=1; }
}
...
[ "$fail" = 0 ] || exit 1
echo "clq-stress: OK — ConcurrentLinkedQueue producer/consumer across carriers (1 & 2, +GC) = $EXP, matches real java"

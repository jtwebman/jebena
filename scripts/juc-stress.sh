...
"$JAVAC" -d "$OUT" "$ROOT"/test/stress/LatchStress.java "$ROOT"/test/stress/SemStress.java "$ROOT/test/diff/Driver.java"
...
for main in LatchStress SemStress; do
  EXP=$("$JAVA" -cp "$OUT" Driver "st.$main" demo 2>/dev/null)
  check "$main" "carriers=1" "JEBENA_CARRIERS=1" 3 "$EXP"
  check "$main" "carriers=4" "JEBENA_CARRIERS=4" 10 "$EXP"
  check "$main" "carriers=4+GC" "JEBENA_GC_INTERVAL=300 JEBENA_CARRIERS=4" 5 "$EXP"
done
[ "$fail" = 0 ] || exit 1
echo "juc-stress: OK — CountDownLatch (8000) + Semaphore (4000) on monitors/wait-notify (carriers 1 & 4, +GC), matches real java"

...
"$JAVAC" -d "$OUT" "$ROOT"/test/stress/LatchStress.java "$ROOT"/test/stress/SemStress.java \
  "$ROOT"/test/stress/RLockStress.java "$ROOT"/test/stress/CondStress.java "$ROOT"/test/stress/BarrierStress.java "$ROOT/test/diff/Driver.java"
...
for main in LatchStress SemStress RLockStress CondStress BarrierStress; do
...
[ "$fail" = 0 ] || exit 1
echo "juc-stress: OK — CountDownLatch + Semaphore + ReentrantLock + Condition + CyclicBarrier on monitors/parking (carriers 1 & 4, +GC), matches real java"

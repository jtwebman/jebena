...
OUT=/tmp/jebena-atomintarr
rm -rf "$OUT"; mkdir -p "$OUT"
bash "$ROOT/scripts/build-jbase.sh" >/dev/null
"$JAVAC" -d "$OUT" "$ROOT/test/stress/AtomIntArrStress.java" "$ROOT/test/diff/Driver.java"
"$ZIG" build --build-file "$ROOT/build.zig" >/dev/null 2>&1
JEBENA="$ROOT/zig-out/bin/jebena"
mapfile -t JBASE < <(find "$ROOT/jbase/out" -name '*.class')
mapfile -t APP < <(ls "$OUT"/st/*.class)

EXP=$("$JAVA" -cp "$OUT" Driver st.AtomIntArrStress demo 2>/dev/null)
fail=0
# per-rep timeout retried ONCE (load-flake tolerance; a genuine hang fails both tries).
onerep() { # env rep
  local out rc got
  out=$(timeout 120 env $1 "$JEBENA" run st/AtomIntArrStress demo "${APP[@]}" "${JBASE[@]}" 2>&1); rc=$?
  if [ $rc -eq 124 ]; then
    out=$(timeout 120 env $1 "$JEBENA" run st/AtomIntArrStress demo "${APP[@]}" "${JBASE[@]}" 2>&1); rc=$?
    [ $rc -eq 124 ] && { echo "atomintarr-stress: FAIL ($1) rep=$2 HANG (twice)"; fail=1; return; }
  fi
  got=$(printf '%s\n' "$out" | sed -n 's/.*demo() = \(-\?[0-9]*\).*/\1/p')
  [ "$got" = "$EXP" ] || { echo "atomintarr-stress: FAIL ($1) rep=$2 jebena=$got java=$EXP"; fail=1; }
}
...
[ "$fail" = 0 ] || exit 1
echo "atomintarr-stress: OK — AtomicIntegerArray getAndAdd/CAS contention across carriers (1 & 2, +GC) = $EXP, matches real java"
